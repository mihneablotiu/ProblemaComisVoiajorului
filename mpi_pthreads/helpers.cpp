...
void *thread_function(void *arg) {
    ...
    int proc_begin = current_struct->proc_begin;
    ...
    for (int i = begin; i < end; i++) {
        ...
        optimal_costs[i - proc_begin] = optimal_cost;
        optimal_paths[i - proc_begin] = optimal_path;
    }
...
void tsp_mpi_worker(int **cost_matrix, int start, int n, int thread_num) {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        thread_structs[i].proc_begin = begin;

        thread_structs[i].begin = begin;
        thread_structs[i].begin += i * (double) size / num_threads;

        thread_structs[i].end = begin;
        thread_structs[i].end += min((i + 1) * (double) size / num_threads, size);

        if (thread_structs[i].end > end) {
            thread_structs[i].end = end;
        }
...
void tsp_mpi_master(int **cost_matrix, int start, int n, int num_procs, int thread_num) {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        thread_structs[i].nr_of_cities = n;
        thread_structs[i].start_node = start;
        thread_structs[i].proc_begin = my_begin;

        thread_structs[i].begin = my_begin;
        thread_structs[i].begin += i * (double) size / num_threads;

        thread_structs[i].end = my_begin;
        thread_structs[i].end += min((i + 1) * (double) size / num_threads, size);

        if (thread_structs[i].end > my_end) {
            thread_structs[i].end = my_end;
        }

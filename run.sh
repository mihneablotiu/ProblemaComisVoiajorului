...
#     for j in {1..3}; do
#         output=$(mpirun -np ${PROCS} --oversubscribe mpi_openmp_implementation ${THREADS} < tests/in/test${i}.in)
...
#     echo "Path: ${last_path}" > tests/out_mpi_openMP/test${i}.out.${PROCS}procs.${THREADS}threads.average
#     echo "Minimum cost: ${last_minimum_cost}" >> tests/out_mpi_openMP/test${i}.out.${PROCS}procs.${THREADS}threads.average
#     echo "Mean total time: ${mean_total_time} seconds" >> tests/out_mpi_openMP/test${i}.out.${PROCS}procs.${THREADS}threads.average

...
void tsp_mpi_master(int **cost_matrix, int start, int n, int num_procs) {
    ...
    int current_cost;
    int *current_path = (int *) malloc((n + 1) * sizeof(*current_path));
    for (int i = 1; i < num_procs; i++) {
        MPI_Recv(&current_cost, ONE, MPI_INT, i, TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(current_path, n + 1, MPI_INT, i, TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
